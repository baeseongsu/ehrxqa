...
@null_if_any
def strftime_fn(fmt, arg):
    """
    Custom implementation of strftime function.
    """
    this = datetime.datetime.fromisoformat(arg).strftime(fmt)
    if fmt.lower() == "%j":
        return int(this)
    else:
        return this
...
def datetime_fn(arg, fmt: str = None) -> datetime:
    """
    ...
    if isinstance(arg, str):
        try:
            ...
            raise ValueError(f"Invalid datetime string: '{arg}'.")
    ...
    if fmt == "start of month":
        ...
        return arg.replace(month=1, day=1)
    elif fmt == "start of day":
        return arg.replace(hour=0, minute=0, second=0, microsecond=0)
    else:
        try:
            number, unit = fmt.split(" ")
            ...
            if unit in ["day", "days"]:
                return arg + datetime.timedelta(days=number)
            elif unit in ["month", "months"]:
                year = arg.year + (arg.month + number - 1) // 12
                month = (arg.month + number - 1) % 12 + 1
                day = min(arg.day, (datetime.datetime(year, month + 1, 1) - datetime.timedelta(days=1)).day)
                return datetime.datetime(year, month, day, arg.hour, arg.minute, arg.second, arg.microsecond)
            elif unit in ["year", "years"]:
                year = arg.year + number
                day = min(arg.day, (datetime.datetime(year + 1, arg.month, 1) - datetime.timedelta(days=1)).day)
                return datetime.datetime(year, arg.month, day, arg.hour, arg.minute, arg.second, arg.microsecond)
            else:
